    ...
    for acquisition_size in [10]
        ...
        for AL_iteration = 1:round(Int, total_pool_samples / acquisition_size, RoundUp)
            ...
            elseif size(new_pool)[2] < acquisition_size && size(new_pool)[2] > 0
				prior = (new_prior[1], sigma)
				new_pool, new_prior, param_matrix, new_training_data = uncertainty_sampling(prior, new_pool, new_training_data, input_size, n_output, param_matrix, AL_iteration, test, name_exp, acquisition_size=size(new_pool)[2], nsteps=1000)

				prior_random = (new_prior_random[1], sigma)
				new_pool_random, prior_random, param_matrix_random, new_training_data_random = random_sampling(new_prior_random, new_pool_random, new_training_data_random, input_size, n_output, param_matrix_random, AL_iteration, test, name_exp_random, acquisition_size=size(new_pool_random)[2], nsteps=1000)

				println("Pool exhausted")
			elseif size(new_pool)[2] > acquisition_size
				prior = (new_prior[1], sigma)
				new_pool, new_prior, param_matrix, new_training_data = uncertainty_sampling(prior, new_pool, new_training_data, input_size, n_output, param_matrix, AL_iteration, test, name_exp, acquisition_size=acquisition_size, nsteps=1000)
				
				prior_random = (new_prior_random[1], sigma)
				new_pool_random, new_prior_random, param_matrix_random, new_training_data_random = random_sampling(prior_random, new_pool_random, new_training_data_random, input_size, n_output, param_matrix_random, AL_iteration, test, name_exp_random, acquisition_size=acquisition_size, nsteps=1000)
            end
            AL_iteration += 1
        end
    end
...
PATH = @__DIR__
cd(PATH)
experiments = "4_experiments_keeping_sigma_constant"
