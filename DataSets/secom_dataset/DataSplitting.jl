...
df = vcat(train, test)
rename!(df, :target => :label)
df[df.label.==-1, :label] .= 2
using Random
function train_validate_test(df; v=0.6, t=0.8)
	...
	CSV.write("../secomPCA_dataset/train.csv", DataFrame(hcat(permutedims(train_x_transformed), train.label), :auto))
	CSV.write("../secomPCA_dataset/test.csv", DataFrame(hcat(permutedims(test_x_transformed), test.label), :auto))
