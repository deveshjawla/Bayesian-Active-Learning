...
experiment = "Simple_Functions_MCMC"
activation_function = "tanh"
function_names = ["Cosine"]#  "Cosine", "Polynomial", "Exponential", "Logarithmic",
...
for function_name in function_names
    ...
    #     nn = Chain(
    #         Parallel(vcat, Dense(w11, b11,mish), Dense(w12, b12, mish), Dense(w13, b13, mish),), Parallel(vcat, Dense(w21, b21, mish), Dense(w22, b22, mish), Dense(w23, b23,mish),), Dense(w31, b31))
    #     return nn
    # end
    ...
    function nn_forward(nn_params::AbstractVector)
        ...
        nn = Chain(Dense(W₁, b₁, tanh), Dense(W₂, b₂, tanh), Dense(W3, b3, tanh), Dense(W4, b4, tanh), Dense(W5, b5))
        return nn
    end
    ...
    @model bayes_nn(xs, ys, num_params, prior_variances) = begin
    ...
    N = 1000
    ch1 = sample(bayes_nn(hcat(xs...), ys, num_params, glorot_normal), NUTS(), N)
    ...
    scatter!(xs, ys, color=:green, markersize=0.5, label="Training data", markerstrokecolor=:green)
    plot!(Xline, map(x -> f(x, function_name), Xline), label="Truth", color=:green)

    savefig(plt, "./$(experiment)/Plots/$(function_name)_$(activation_function).png")
