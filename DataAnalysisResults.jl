...
col_names = [:Dataset, :Prior, :BalAcc, :MacroF1, :Time]
results_df = DataFrame([[] for _ in col_names], col_names)
                ...
                for acq_func in acq_functions
					for experiment in experiments
                    	for temperature in temperatures
                            stats_df = Array{Any}(undef, 3, n_folds)
                            for fold in 1:n_folds
                                num_mcsteps = 100
                                num_chains = 2
                                pipeline_name = "./Experiments/$(experiment)/$(acquisition_size)_$(acq_func)_$(prior_variance)_$(likelihood_name)_$(prior_informativeness)_$(temperature)_$(fold)_$(num_chains)_$(num_mcsteps)/classification_performance"

                                results = readdlm("$(pipeline_name)/$(n_acq_steps).csv",',')
                                ...
                                stats_df[:, fold] = vcat(results[2:3,2], elapsed[1])
                            end
                            ...
                            push!(results_df, vcat(dataset, experiment, stats_df_))
                        end
                    end
                end
            end
...
CSV.write("../../../results00.csv", results_df)