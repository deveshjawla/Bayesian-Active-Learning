"""
Returns means, stds, classifications, majority_voting, majority_ensemble_majority
...
majority_voting, majority_ensemble_majority are averaged classifications of each chain, if the majority i.e more than 0.5 chains voted in favor then we have a vote in favor of 1. the ensemble_majority here means how many chains on average voted in favor of 1. majority vote is just rounding of majority ensemble_majority.
"""
function pred_analyzer(test_xs, test_ys, params_set, threshold)::Tuple{Array{Float32},Array{Float32},Array{Int},Array{Int},Array{Float32}}
    ...
    for (test_x, test_y) in zip(eachrow(test_xs), test_ys)
        ...
        majority_ensemble_majority_ = mean(individual_classifications)
        ...
        append!(majority_ensemble_majority, majority_ensemble_majority_)
    end
    ...
    # end
    return means, stds, classifications, majority_voting, majority_ensemble_majority
end
