...
#TODO - a better way to avoid duplicates is to look at the prediction vector if the entropy is the same, if the prediction vectors of two samples are also same then we can say that the two samples are copies of each other.
"""
...
function top_k_acquisition(pool_scores::Vector, acquisition_size::Int; descending=false)
    df = DataFrame(Scores=pool_scores, Sample_indices=collect(1:lastindex(pool_scores)))
    # Sorting the DataFrame based on Scores
    sorted_df = sort(cut_df, :Scores, rev=descending)
    top_k = sorted_df[1:acquisition_size, :Sample_indices]
