...
# function normalize_dataset(x)
#     return (x .- minimum(x)) ./ (maximum(x) - minimum(x) .+ 0.000001)
# end

# mapcols!(x -> normalize_dataset(x), X_train)
# sum(count.(isnan, eachcol(X_train)))
# mapcols!(x -> normalize_dataset(x), test_x)
# sum(count.(isnan, eachcol(test_x)))
...
# train_max = mean(Matrix(X_train), dims=1)
# train_min = std(Matrix(X_train), dims=1)

# X_train = standardize(Matrix(X_train), train_max, train_min)
# test_x = standardize(Matrix(test_x), train_max, train_min)

# train = hcat(X_train, Matrix(y_train))
...
test_xy[test_xy.target.==-1, :target] .= 2
