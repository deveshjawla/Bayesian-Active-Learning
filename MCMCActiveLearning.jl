...
for experiment in experiments
    ...
    for (dataset, inout_dims, acquisition_size, n_folds, n_acq_steps) in zip(datasets, list_inout_dims, acquisition_sizes, list_n_folds, list_acq_steps)
        ...
        #     else
        #         kpi_names = vcat([:AcquisitionSize, :ClassDistEntropy, :Accuracy, :F1, :AcquisitionFunction, :Temperature, :Experiment, :CumTrainedSize, :PriorInformativeness, :PriorVariance, :LikelihoodName], Symbol.(class_names), Symbol.(class_names), :CumCDE, :Elapsed, :OOBRhat, :AcceptanceRate, :NumericalErrors, :AvgESS, :MaxPSRF)
        #     end
        ...
        #     if n_output == 1
        #         auc_per_fold(fold, df_fold, :PriorInformativeness, :MSE, :Elapsed)
        #     else
        #         auc_per_fold(fold, df_fold, :AcquisitionFunction, :Accuracy, :Elapsed)
        #     end
        ...
        if n_output == 1
            ...
            mean_std_by_group(df_folds, :AcquisitionFunction, :CumTrainedSize; list_measurables=[:Accuracy, :F1, :Elapsed, :AcceptanceRate, :NumericalErrors])
        end
		...
		for (i, j, k, l) in zip(list_plotting_measurables, list_plotting_measurables_mean, list_plotting_measurables_std, list_normalised_or_not)
			plotting_measurable_variable(experiment, :AcquisitionFunction, acq_functions, dataset, :CumTrainedSize, i, j, k, l)
        end
    end
end