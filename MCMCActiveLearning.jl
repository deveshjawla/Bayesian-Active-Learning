...
for dataset in datasets
            ...
            for acq_func in acq_functions
                for acquisition_size in acquisition_sizes
                    ...
                    for AL_iteration = 1:n_acq_steps
                        ...
                        if AL_iteration == 1
                            new_pool, param_matrix, new_training_data, last_acc, last_elapsed, location_posterior = bnn_query(prior, pool, new_training_data, input_size, n_output, param_matrix, AL_iteration, test, experiment_name, pipeline_name, acquisition_size, num_mcsteps, num_chains, "Initial", mcmc_init_params)
                        ...
                        elseif lastindex(new_pool[2]) > acquisition_size
                            ...
                            # else
                                # new_prior = prior
                            # end
                            new_pool, param_matrix, new_training_data, last_acc, last_elapsed, location_posterior = bnn_query(new_prior, new_pool, new_training_data, input_size, n_output, param_matrix, AL_iteration, test, experiment_name, pipeline_name, acquisition_size, num_mcsteps, num_chains, acq_func, mcmc_init_params)
                            mcmc_init_params = deepcopy(location_posterior)
                        ...
                        elseif lastindex(new_pool[2]) <= acquisition_size && lastindex(new_pool[2]) > 0
                            ...
                            new_pool, param_matrix, new_training_data, last_acc, last_elapsed, location_posterior = bnn_query(new_prior, new_pool, new_training_data, input_size, n_output, param_matrix, AL_iteration, test, experiment_name, pipeline_name, lastindex(new_pool[2]), num_mcsteps, num_chains, acq_func, mcmc_init_params)
                            mcmc_init_params = deepcopy(location_posterior)
                    ...
                    kpi_2 = permutedims(reduce(vcat, [permutedims(repeat([experiment], n_acq_steps)), confidences_list, cum_acq_train_vector, kpi]))
    ...
    # df1 = CSV.read("./$(experiment_name)/DNN_df.csv", DataFrame, header=1)
    # df2 = CSV.read("./$(experiment_name)/Informed_df.csv", DataFrame, header=1)
    # df3 = CSV.read("./$(experiment_name)/XGB_df.csv", DataFrame, header=1)
    # df = vcat(df1, df2, df3)
    ...
    fig1a |> PNG("./$(experiment_name)/Accuracy_$(dataset)_$(experiment_name).png", dpi=600)
    fig1aa |> PNG("./$(experiment_name)/Confidence_$(dataset)_$(experiment_name).png", dpi=600)
    fig1b |> PNG("./$(experiment_name)/TrainingTime_$(dataset)_$(experiment_name).png", dpi=600)
    # end
