...
using Turing
num_chains = 2
datasets = ["iris", "creditdefault", "banknote", "creditfraud", "stroke", "coalmine"]
acq_functions = ["Random", "PowerBALD", "BayesianUncertainty"]
# experiments = ["Informed", "UnInformed"]
...
theme = Theme(major_label_font_size=16pt, minor_label_font_size=14pt, key_title_font_size=14pt, key_label_font_size=12pt, key_position=:right, colorkey_swatch_shape=:circle, key_swatch_size=12pt)
...
for dataset in datasets
    experiment_name = "Various_Acquisition_Fucntions"
    PATH = @__DIR__
    ...
    #     init_variance = vcat(sqrt(2 / (input_size + l1)) * ones(nl1), sqrt(2 / (l1 + l2)) * ones(nl2), sqrt(2 / (l2 + n_output)) * ones(n_output_layer))
    #     # # Naive initialisation
    #     # init_variance = ones(num_params)

    #     # Only for the Hidden, When using Hyper Prior
    #     # init_variance = vcat(sqrt(2 / (input_size + l1)) * ones(l1), sqrt(2 / (l1 + l2)) * ones(nl2), sqrt(2 / (l2 + n_output)) * ones(n_output_layer))
    # end
    ...
    #             pipeline_name = "$(acq_func)_with_$(num_mcsteps)_MCsteps"
    #             mkpath("./$(experiment_name)/$(pipeline_name)/predictions")
    #             mkpath("./$(experiment_name)/$(pipeline_name)/hyperpriors")
    ...
    #             n_acq_steps = 4#round(Int, total_pool_samples / acquisition_size, RoundUp)
    #             prior = (zeros(num_params), init_variance)
    ...
    #                 if AL_iteration == 1
    #                     new_pool, param_matrix, new_training_data, last_acc, last_elapsed, location_posterior = bnn_query(prior, pool, new_training_data, input_size, n_output, param_matrix, AL_iteration, test, experiment_name, pipeline_name, acquisition_size, num_mcsteps, num_chains, acq_func)
    #                 elseif lastindex(new_pool[2]) > acquisition_size
    #                     # if experiment == "Informed"
    #                         # new_prior = (location_posterior, init_variance)
    #                     # else
    #                         new_prior = prior
    #                     # end
    #                     new_pool, param_matrix, new_training_data, last_acc, last_elapsed, location_posterior = bnn_query(new_prior, new_pool, new_training_data, input_size, n_output, param_matrix, AL_iteration, test, experiment_name, pipeline_name, acquisition_size, num_mcsteps, num_chains, acq_func)
    ...
    #                     new_pool, param_matrix, new_training_data, last_acc, last_elapsed, location_posterior = bnn_query(new_prior, new_pool, new_training_data, input_size, n_output, param_matrix, AL_iteration, test, experiment_name, pipeline_name, lastindex(new_pool[2]), num_mcsteps, num_chains, acq_func)
    ...
    # df1 = CSV.read("./$(experiment_name)/Informed_df.csv", DataFrame, header=1)
    # df2 = CSV.read("./$(experiment_name)/UnInformed_df.csv", DataFrame, header=1)
    # df = vcat(df1, df2)
    df = CSV.read("./$(experiment_name)/df.csv", DataFrame, header=1)
    # for (j, i) in enumerate(groupby(df, :AcquisitionSize))
    fig1a = Gadfly.plot(df, x=:CumTrainedSize, y=:Accuracy, color=:AcquisitionFunction, Geom.point, Geom.line, yintercept=[0.5], Geom.hline(color=["red"], size=[1mm]), Guide.xlabel("Cumulative Training Size"), Coord.cartesian(xmin=df.AcquisitionSize[1], ymin=0.5, ymax=1.0))
    ...
    fig1a |> PNG("./$(experiment_name)/Accuracy.png")
    fig1aa |> PNG("./$(experiment_name)/Confidence.png")
    fig1b |> PNG("./$(experiment_name)/TrainingTime.png")
    # end
