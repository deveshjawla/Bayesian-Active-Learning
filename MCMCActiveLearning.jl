...
experiments = ["InformedvsUninformed"]
datasets = ["yeast1996"]#20, 20, 10, 10, 20, 20, 10, 40
# acquisition_sizes = [20, 10, 10, 20, 20, 10, 40]#"stroke", "adult1994", "banknote2012", "creditfraud", "creditdefault2005", "coalmineseismicbumps",  "iris1988", "yeast1996"
acquisition_sizes = [40]
...
list_n_folds = [3]#4, 5, 5, 5, 5, 2, 3, 3

list_class_balancing = ["UnBalancedAcquisition"] #"BalancedBinaryAcquisition"
list_prior_informativeness = ["UnInformedPrior", "InformedPrior", "NoInit"] # "UnInformedPrior", "InformedPrior"
list_prior_std_name = ["GlorotPrior"] # "UnScaledPrior", 
...
temperatures = [nothing] # 1.0, 0.1, 0.001
...
for experiment in experiments
    ...
    for (dataset, inout_dims, acquisition_size, n_folds) in zip(datasets, list_inout_dims, acquisition_sizes, list_n_folds)
        ...
        #     kpi_df = Array{Any}(missing, 0, 14 + 2 * n_output)
        ...
        #             for prior_std_name in list_prior_std_name
        #                 @everywhere prior_std_name = $prior_std_name
        ...
        #                                 if prior_std_name == "UnScaledPrior"
        #                                     # Naive initialisation
        #                                     prior_std = ones(num_params)
        #                                 else
        #                                     # GlorotNormal initialisation
        #                                     prior_std = vcat(sqrt(2 / (n_input + l1)) * ones(nl1), sqrt(2 / (l1 + l2)) * ones(nl2), sqrt(2 / (l2 + n_output)) * ones(n_output_layer))
        #                                 end
        #                             end
        ...
        #                                 pipeline_name = "$(acquisition_size)_$(acq_func)_$(prior_std_name)_$(likelihood_name)_$(prior_informativeness)_$(temperature)_$(fold)_$(num_chains)_$(num_mcsteps)"
        #                                 # pipeline_name = "$(acquisition_size)_$(acq_func)_$(prior_std_name)_$(likelihood_name)_$(prior_informativeness)_$(temperature)_$(num_chains)_$(num_mcsteps)"
        ...
        #                                 n_acq_steps = 5#round(Int, total_pool_samples / acquisition_size, RoundUp)
        ...
        #                                             new_prior = (location_posterior, prior_std)
        #                                         end
        #                                         new_pool, param_matrix, map_matrix, new_training_data, last_acc, last_elapsed, location_posterior = bnn_query(new_prior, new_pool, new_training_data, n_input, n_output, param_matrix, map_matrix, AL_iteration, test, experiment, pipeline_name, lastindex(new_pool[2]), num_mcsteps, num_chains, acq_func, mcmc_init_params, temperature, class_balancing, prior_informativeness, prior_std_name, likelihood_name)
        #                                         mcmc_init_params = deepcopy(location_posterior)
        ...
        #                                         performance_data[7, al_step] = temperature
        #                                         performance_data[8, al_step] = experiment

        #                                         #Cumulative Training Size
        #                                         if al_step == 1
        #                                             performance_data[9, al_step] = m[1, 2]
        #                                         else
        #                                             performance_data[9, al_step] = performance_data[9, al_step-1] + m[1, 2]
        #                                         end


        #                                         performance_data[10, al_step], performance_data[11, al_step], performance_data[12, al_step], performance_data[13, al_step] = class_balancing, prior_informativeness, prior_std_name, likelihood_name
        ...
        #     kpi_names = vcat([:AcquisitionSize, :ClassDistEntropy, :Accuracy, :EnsembleMajority, :Elapsed, :AcquisitionFunction, :Temperature, :Experiment, :CumTrainedSize, :ClassBalancing, :PriorInformativeness, :PriorStd, :LikelihoodName], Symbol.(class_names), Symbol.(class_names), :CumCDE)
        ...
        #     prior_name = i.PriorInformativeness[1]
        #     CSV.write("./Experiments/$(experiment)/mean_std_acc$(prior_name).csv", mean_std_acc)
        #     CSV.write("./Experiments/$(experiment)/mean_std_time$(prior_name).csv", mean_std_time)
        #     CSV.write("./Experiments/$(experiment)/mean_std_ensemble_majority$(prior_name).csv", mean_std_ensemble_majority)
        # end
        ...
        # fig1a = Gadfly.plot(df, x=:CumTrainedSize, y=:Accuracy, color=:PriorInformativeness, Geom.point, Geom.line, yintercept=[0.5], Geom.hline(color=["red"], size=[1mm]), Guide.xlabel("Cumulative Training Size"), Coord.cartesian(xmin=df.AcquisitionSize[1], ymin=0.0, ymax=1.0))

        # fig1aa = Gadfly.plot(df, x=:CumTrainedSize, y=:EnsembleMajority, color=:PriorInformativeness, Geom.point, Geom.line, yintercept=[0.5], Geom.hline(color=["red"], size=[1mm]), Guide.xlabel("Cumulative Training Size"), Coord.cartesian(xmin=df.AcquisitionSize[1], ymin=0.5, ymax=1.0))

        # fig1b = Gadfly.plot(df, x=:CumTrainedSize, y=:Elapsed, color=:PriorInformativeness, Geom.point, Geom.line, Guide.ylabel("Training (seconds)"), Guide.xlabel(nothing), Coord.cartesian(xmin=0))
        ...
        # fig1c = plot(df, x=:CumTrainedSize, y=:ClassDistEntropy, color=:PriorInformativeness, Geom.point, Geom.line, Guide.ylabel("Class Distribution Entropy"), Guide.xlabel(nothing), Coord.cartesian(xmin=df.AcquisitionSize[1], ymin=0.0, ymax=1.0))

        # fig1cc = plot(df, x=:CumTrainedSize, y=:CumCDE, color=:PriorInformativeness, Geom.point, Geom.line, Guide.ylabel("Cumulative CDE"), Guide.xlabel(nothing), Coord.cartesian(xmin=df.AcquisitionSize[1], ymin=0.0, ymax=1.0))
