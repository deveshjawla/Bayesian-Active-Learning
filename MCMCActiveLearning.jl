...
using Turing
num_chains = 2
datasets = ["coalmine", "banknote", "iris", "creditdefault", "creditfault", "stroke"]
acq_functions = ["TopKBayesian"]
prior_specifications = ["HeGlorot", "Naive"]
...
for dataset in datasets
    for prior_specification in prior_specifications
		@everywhere prior_specification = $prior_specification
        aocs = []
        n_aocs = []
        acquisition_sizes = [20]

        experiment_name = "HeGlorot_vs_Naive"
        PATH = @__DIR__
        ...
        @everywhere begin
            ...
            if prior_specification == "HeGlorot"
                # HeGlorot initialisation
                init_params = vcat(sqrt(2 / (input_size + l1)) * ones(nl1), sqrt(2 / (l1 + l2)) * ones(nl2), sqrt(2 / (l2 + n_output)) * ones(n_output_layer))
            elseif prior_specification == "Naive"
                # Naive initialisation
                init_params = ones(num_params)
            end

            # Only for the Hidden, When using Hyper Prior
            # init_params = vcat(sqrt(2 / (input_size + l1)) * ones(l1), sqrt(2 / (l1 + l2)) * ones(nl2), sqrt(2 / (l2 + n_output)) * ones(n_output_layer))
        end
        ...
        let
            ...
            for acq_func in acq_functions
                for acquisition_size in acquisition_sizes
                    ...
                    pipeline_name = "$(prior_specification)_with_$(num_mcsteps)_MCsteps"
                    ...
                    for AL_iteration = 1:n_acq_steps
                        ...
                        elseif lastindex(new_pool[2]) > acquisition_size
                            # new_prior = (new_prior[1], init_params)
        ...
        fig1a = Gadfly.plot(df, x=:CumTrainedSize, y=:Accuracy, color=:AcquisitionFunction, Geom.point, Geom.line, yintercept=[0.5], Geom.hline(color=["red"], size=[1mm]), Guide.xlabel("Cumulative Training Size"), Coord.cartesian(xmin=acquisition_sizes[1], ymin=0.5, ymax=1.0))
        fig1aa = Gadfly.plot(df, x=:CumTrainedSize, y=:Confidence, color=:AcquisitionFunction, Geom.point, Geom.line, yintercept=[0.5], Geom.hline(color=["red"], size=[1mm]), Guide.xlabel("Cumulative Training Size"), Coord.cartesian(xmin=acquisition_sizes[1], ymin=0.5, ymax=1.0))
        ...
        fig1a |> PNG("./$(experiment_name)/$(prior_specification).png")
        fig1aa |> PNG("./$(experiment_name)/$(prior_specification)_confidence.png")
        fig1b |> PNG("./$(experiment_name)/$(prior_specification)_time.png")
        # end
    end
end