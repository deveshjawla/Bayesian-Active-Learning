...
function analyse_uncertainties(TotalSamples::Int64, num_Categories::Int64)::Array{Float16,2}
	...
	C_Maj, C_Min, C_Eq, List_AverageSoftmaxofCorrect, List_AverageSoftmaxofInCorrect = nothing, nothing, nothing, nothing, nothing
    ...
    @showprogress Threads.@threads for i = 1:total_cases
        ...
        PctCorrectByMembers = SMatrix{1,2}(PctCorrectByMembers[1], 1 - PctCorrectByMembers[1])
...
function plotting_correlation_matrices(TotalSamples::Int64, num_Categories::Int64, results::DataFrame)::Nothing
    for df in groupby(results, :Case)
        cols = [:PctCorrectByMembers, :AvgEntropyCorrect, :AvgEntropyInCorrect, :TotalUncertainty, :Aleatoric, :Epistemic, :VarAleatoric]  # define subset
